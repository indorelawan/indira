...
import {
  TActivity,
  TActivityFixedLocationData,
  TActivityFlexibleLocationData,
  TActivityResponse,
  TEventMultipleScheduleData,
  TEventRangeScheduleData,
  TProjectScheduleData,
  TRegularScheduleData,
} from './types';

const getActivities = async () => {
  const res = await axios.get<TActivityResponse>(config.activityUrl);
    ...
    const getSchedules = () => {
      if (activity.type === 'event') {
        if (activity.events_type === 'multiple') {
          ...
          const mapped = sched.map(
            (schedule: TEventMultipleScheduleData) => `Tanggal kegiatan: ${schedule.date}
